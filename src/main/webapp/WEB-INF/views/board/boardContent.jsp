					...
					<c:choose>
						<c:when test="${ fn:length(attachList) gt 0 }"><%--attach > 0 --%>
						 ...
						 <c:forEach var="attach" items="${ attachList }">
						 	...
						 	<%--다운로드할 파일 경로입니다. --%>
						 	 <c:set var="filepath" value="${attach.uploadpath }/${attach.filename }" />
						 		<li>
						 	...
						 	<%--원본 이미지 파일 경로 --%>
						 	<c:set var="fileOringn" value="${ attach.uploadpath }/_${ attach.uuid }_${ attach.filename }" />
						 	<li>
						 		<a href="/display?fileName=${fileOringn }">
